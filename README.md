...
#### Preflight an insights extract: PR count, comment volume, API budget

```sh
gh review-kit insights estimate [--repo REPO] [--state STATE] [--merged] [--since RFC3339] [--until RFC3339] [--labels LABELS] [--comment-types TYPES] [--limit N] [--sample-size N] [--format FORMAT]
```

Estimate how much GitHub API work a future `insights extract` run with the same filters would consume. The command lists matching pull requests (cheap REST pagination), samples a small number of them to measure average comment volume per PR, and reports the projected total comments, projected API calls, and current rate-limit headroom. Use it before kicking off a large extraction to avoid hitting secondary rate limits or running out of REST quota mid-run.
...
```sh
gh review-kit insights extract --dataset DIR [--repo REPO] [--state STATE] [--merged] [--since RFC3339] [--until RFC3339] [--labels LABELS] [--comment-types TYPES] [--include-bots] [--min-length N] [--path PREFIX] [--limit N] [--no-redact] [--update]
```
...
The dataset directory is the unit shared by every `insights` subcommand and contains:
...
```sh
gh review-kit insights validate --dataset DIR [--strict] [--format FORMAT]
```
...
```sh
gh review-kit insights stats --dataset DIR [--group-by KEY] [--top N] [--min-count N] [--format FORMAT]
```
...
```sh
gh review-kit insights sample --dataset DIR [--group-by KEY] [--per-group N] [--strategy STRATEGY] [--seed N] [--output FILE] [--format FORMAT] [--comment-types TYPES] [--review-states STATES] [--authors AUTHORS] [--path PREFIX] [--since RFC3339] [--until RFC3339] [--min-length N] [--include-bots]
```
...
```sh
gh review-kit insights bundle --dataset DIR --output-dir DIR [--group-by KEY] [--max-records N] [--max-bytes N] [--comment-types TYPES] [--review-states STATES] [--authors AUTHORS] [--path PREFIX] [--since RFC3339] [--until RFC3339] [--min-length N] [--include-bots] [--format FORMAT]
```
...
```sh
gh review-kit insights suggest-rules --dataset DIR [--topics-file FILE] [--min-count N] [--min-reviewers N] [--examples N] [--output FILE] [--format FORMAT] [--comment-types TYPES] [--review-states STATES] [--path PREFIX] [--since RFC3339] [--until RFC3339] [--min-length N] [--include-bots]
```
...
```sh
gh review-kit insights report --dataset DIR [--topics-file FILE] [--format FORMAT] [--output FILE] [--stats-top N] [--min-count N] [--min-reviewers N] [--examples N] [--comment-types TYPES] [--review-states STATES] [--path PREFIX] [--since RFC3339] [--until RFC3339] [--min-length N] [--include-bots]
```
