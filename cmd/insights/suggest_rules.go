...
// NewSuggestRulesCmd creates the 'insights suggest-rules' command.
...
URLs. No fuzzy clustering or embeddings are used; the output is reproducible.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			var topics *insights.TopicSet
			if topicsFile != "" {
				ts, err := insights.LoadTopicSet(topicsFile)
				if err != nil {
					return err
				}
				topics = ts
			...
			result, err := insights.SuggestRules(dataset, insights.SuggestRulesOptions{
				Topics: topics,
				Filters: insights.SampleFilters{
					CommentTypes: types,
...
func writeRulesOutput(cmd *cobra.Command, r *insights.SuggestRulesResult, output, format string, exporter cmdutil.Exporter) error {
	w := cmd.OutOrStdout()
	if output != "" {
	...
	switch format {
	case "markdown":
		report := &insights.Report{Dataset: r.Dataset, Rules: r, Stats: map[string]*insights.StatsResult{}}
		return insights.WriteReportMarkdown(w, report)
	default:
