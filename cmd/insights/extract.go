...
// NewExtractCmd creates the 'insights extract' command.
...
applied by default; pass --no-redact to opt out.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			ds, err := insights.OpenDataset(dataset, filters)
			if err != nil {
				return fmt.Errorf("failed to open dataset %q: %w", dataset, err)
			}
			defer ds.Close()

			opts := insights.ExtractOptions{
				Repo:         repository,
				State:        state,
				MergedOnly:   mergedOnly,
				Since:        since,
...
func stringifyTypes(types []insights.CommentType) []string {
