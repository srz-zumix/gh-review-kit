...
// NewSampleCmd creates the 'insights sample' command.
...
Strategies:
  ...
  random          random with --seed (deterministic when seeded)`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			strat, err := insights.ParseSampleStrategy(strategy)
			if err != nil {
				return err
			}

			opts := insights.SampleOptions{
			...
			records, err := insights.Sample(dataset, opts)
			if err != nil {
...
func writeSampleOutput(cmd *cobra.Command, records []*insights.Comment, output, format string, exporter cmdutil.Exporter) error {
