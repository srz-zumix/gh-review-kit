...
// NewEstimateCmd creates the 'insights estimate' command.
func NewEstimateCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "estimate",
		Short: "Preflight an insights extract: PR count, comment volume, API budget",
		Long: `Estimate how much GitHub API work a future 'insights extract' run with the
...
limits or running out of REST quota mid-run.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			result, err := insights.Estimate(context.Background(), client, insights.EstimateOptions{
				Repo:         repository,
				State:        state,
...
func writeEstimateOutput(r *render.Renderer, result *insights.EstimateResult) error {
	if r.HasExporter() {
